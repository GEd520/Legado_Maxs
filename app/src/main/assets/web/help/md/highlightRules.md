...
data class HighlightRule(
    ...
    var bgSpacingH: Float,           // 背景图左右间距（em，默认 0；正数向外扩、负数向内收）
    var bgSpacingV: Float            // 背景图上下间距（em，默认 0）
)
```
...
| `bgSpacingH` | 0 | 左右间距（em，随字号缩放）：**正数**把背景向外撑大、离文字更远，**负数**向内收（范围 −1.00 ~ +0.50em） |
| `bgSpacingV` | 0 | 上下间距（em）：正数向外撑大，负数向内收（范围 −1.00 ~ +0.50em） |
