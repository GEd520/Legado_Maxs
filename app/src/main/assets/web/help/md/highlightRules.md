...
data class HighlightRule(
    ...
    var bgSpacingH: Float,           // 背景图左右间距（em，默认 0；正数向外扩、负数向内收，范围 -1~1）
    var bgSpacingV: Float            // 背景图上下间距（em，默认 0；范围 -0.5~0.5）
)
```
...
#### 外扩策略与间距（bgBleedMode / bgSpacingH / bgSpacingV）

同在"九宫格调整"弹窗内调整，用于控制背景图要在匹配文字之外扩多少。

| 参数 | 默认 | 含义 |
|---|------|------|
| `bgBleedMode` | 智能 | 自动外扩策略：**严格** = 不外扩，背景只覆盖匹配到的文字；**智能** = 只占用邻接的空白；**强制** = 按四边分割比例向外扩展（即原来的"包裹文字"效果），同时**自动把左右邻字推开一个正文字距**，邻字不会被背景盖住 |
| `bgSpacingH` | 0 | 左右间距（em，随字号缩放）：**正数**把背景向外撑大、离文字更远，**负数**向内收（范围 −1.00 ~ +1.00em） |
| `bgSpacingV` | 0 | 上下间距（em）：正数向外撑大，负数向内收（范围 −0.50 ~ +0.50em） |
...
|---|---|---|
| 严格 | 0，绝不外扩 | 0 |
| 智能 | 邻接字符是空白（空格/制表符）时，借用该空白宽度的 80%；匹配段排到行末（段末）时，借右侧到正文边界之间的空白（最多一个字符宽）；邻字有字形时 0 | 吃掉一半行距（行距越大背景越高），不会压到上下行的字形 |
| 强制 | 四角/边条厚度，但**最多一个字宽（1em）**，并把左右邻字各向外推开"外扩量 + 一个正文字距 − 邻字原有空隙" | 四角/边条厚度，但**最多吃掉一半行距** |

> **强制模式下邻字会被推开**：推开的距离作为**真实排版宽度**参与断行与两端对齐，所以剩下的文字仍然整齐（不会溢出右边距，也不会参差不齐）；单侧最多让出 1em，避免极端分割比例把整行挤爆。"一个正文字距"取自阅读设置里的「字间距」，不需要额外配置。
